...
code-server editor preferences, keybindings, and extension IDs are recreated from `config/code-server-settings.json.template`, `config/code-server-keybindings.json.template`, and `config/code-server-extensions.txt`. The settings template enables terminal images (`terminal.integrated.enableImages`) with GPU acceleration set to `auto`. Extension caches, workspace state, session data, and `globalStorage` are intentionally not retained.
